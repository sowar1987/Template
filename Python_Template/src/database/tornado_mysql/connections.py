...
from .converters import (escape_item, encoders, decoders, escape_string,
                         through)
from .err import (raise_mysql_exception, Warning, Error, InterfaceError,
...
TEXT_TYPES = set([
    FIELD_TYPE.BIT, FIELD_TYPE.BLOB, FIELD_TYPE.LONG_BLOB,
    FIELD_TYPE.MEDIUM_BLOB, FIELD_TYPE.STRING, FIELD_TYPE.TINY_BLOB,
    FIELD_TYPE.VAR_STRING, FIELD_TYPE.VARCHAR
])
...
class EOFPacketWrapper(object):
    ...
    def __init__(self, from_packet):
        ...
        self.packet = from_packet
        self.warning_count, self.server_status = self.packet.read_struct(
            '<xhh')
...
class Connection(object):
    ...
    def set_charset(self, charset):
        ...
        yield self._execute_command(COMMAND.COM_QUERY,
    ...
    def _read_packet(self, packet_type=MysqlPacket):
        ...
        try:
            ...
            raise OperationalError(2006,
                                   "MySQL server has gone away (%s)" % (e, ))
...
class MySQLResult(object):
    ...
    def _read_rowdata_packet_unbuffered(self):
        ...
        self.rows = (
            row, )  # rows should tuple of row for MySQL-python compatibility.
        ...
        try:
            with open(self.filename, 'rb') as open_file:
                ...
                while True:
                    ...
                    if not chunk:
                        break
                    packet = struct.pack('<i',
                                         len(chunk))[:3] + int2byte(seq_id)
